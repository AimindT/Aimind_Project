...
# Define un modelo para la entrada del usuario
class UserInput(BaseModel):
    text: str
    session_id: str = None  # Sesión opcional para continuidad

# Sesiones en memoria (podrías usar Redis o una BD si necesitas persistencia real)
...
def model(user_input: str, session_id: str = None):
    ...
    # Mantiene el historial limitado a las últimas 20 interacciones
    messages = sessions[session_id][-20:]
    ...
    chat_completion = client.chat.completions.create(
        messages=[
            ...
            {"role": "system", "content": "No abrumes al usuario con tus respuestas, es preferible respuestas cortas y eficientes y no mensajes largos y agotado."}, 

            # ------ Numero de prevencion contra suicidio y apoyo emocional  ------
            {"role": "system", "content": "Si consideras que el usuario necesita apoyo emocional urgente y proveniente de una persona real, recomienda el siguiente numero telefonico de Mexico (800 911 2000)."},
        ...
        temperature=0.7, #creatividad respuestas
        max_tokens=250,  # Limita la longitud de las respuestas
    )
...
# Ruta principal para asegurarse de que la API está funcionando
@app.get("/")
def read_root():
    return {"message": "¡Bienvenido a AIMIND! ¿En qué te puedo ayudar hoy?"}

# Ruta para interactuar con el psicólogo virtual
@app.post("/chat/")
async def chat(input: UserInput):
    response, session_id = model(input.text, input.session_id)
